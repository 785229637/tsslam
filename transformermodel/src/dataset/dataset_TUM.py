...
class DatasetTUMRGBD(IterableDataset):
    ...
    def __init__(
        ...
        # cfg.name= "re10k"
        cfg.root = Path('/home/ben/3dgsslam/splat-depth/SplaTam_v1/data/TUM_RGBD')
        cfg.image_size  = (480, 640)
        cfg.sequence ="rgbd_dataset_freiburg1_desk2"
    ...
    #             if len(parts) >= 2:
    #                 rgb_timestamps.append(float(parts[0]))
    ...
    #             q = np.array([float(parts[7]), float(parts[4]), float(parts[5]), float(parts[6])])
    #             pose[:3, :3] = self.quaternion_to_rotation_matrix(q)
    #             # quat = parts[4:]
    #             # trans = parts[1:4]
    #             # T = trimesh.transformations.quaternion_matrix(np.roll(quat, 1))
    #             # T[:3, 3] = trans
    #             pose_data.append((timestamp, pose))
    ...
    #         closest_idx = np.argmin(np.abs(np.array(pose_timestamps) - timestamp))
    #         closest_pose = pose_data[closest_idx]
            

    #         if abs(closest_pose[0] - timestamp) < self.cfg.max_time_diff:
    #             img_path = self.rgb_path / f"{timestamp:.6f}.png"
    #             aligned_data.append((img_path, closest_pose[1]))
        
    #     print(f"Aligned {len(aligned_data)} images with poses for sequence: {self.cfg.sequence}")
    #     return aligned_data
    

    def load_and_align_data(self) -> List[Tuple[Path, np.ndarray]]:

        rgb_timestamps = self.load_timestamps(self.rgb_timestamps_file)
        

        depth_timestamps = self.load_timestamps(self.depth_timestamps_file)
        ...
        for rgb_ts in rgb_timestamps:
            ...
            depth_idx = np.argmin(np.abs(np.array(depth_timestamps) - rgb_ts))
            depth_ts = depth_timestamps[depth_idx]
            depth_diff = abs(depth_ts - rgb_ts)
            ...
            if depth_diff < self.cfg.max_time_diff and pose_diff < self.cfg.max_time_diff:
                img_path = self.rgb_path / f"{rgb_ts:.6f}.png"
    ...
    def load_timestamps(self, file_path: Path) -> List[float]:
        timestamps = []
    ...
    def __iter__(self):
        ...
        norm_intrinsics = torch.tensor([
            [520.908620/640, 0, 325.141442/640],
            [0, 521.007327/480, 249.701764/480],
            [0, 0, 1]
        ], dtype=torch.float32)
