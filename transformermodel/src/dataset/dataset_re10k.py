...
class DatasetRE10k(IterableDataset):
    ...
    def __init__(
        ...
        self.prev_last_context_index = None
        self.prev_scene = None
        self.historical_frames = {}
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                if times_per_scene > 1:  # specifically for DTU
                    ...
                    scene = example["key"]
                for scene_idx in range(0,len(example["images"])-21,20):
                    context_indices = torch.tensor([scene_idx, scene_idx+20])
                    target_indices = torch.tensor([scene_idx+20])
                # try:
