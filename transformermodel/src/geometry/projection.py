...
#     indices = [torch.arange(length, device=device) for length in shape]
#     stacked_indices = torch.stack(torch.meshgrid(*indices, indexing="ij"), dim=-1)  # [H, W, 2]
    

#     coordinates = [(idx + 0.5) / length for idx, length in zip(indices, shape)]
#     coordinates = reversed(coordinates)
#     coordinates = torch.stack(torch.meshgrid(*coordinates, indexing="xy"), dim=-1)  # [H, W, 2]
    


#     selected_indices = stacked_indices[mask == 1]  # [num_selected, 2]
#     selected_coordinates = coordinates[mask == 1]   # [num_selected, 2]
